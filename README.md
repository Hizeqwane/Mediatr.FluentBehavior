...
services.AddTransient(typeof(IMediatrPipelineBuilder<>), typeof(MediatorPipelineBuilder<>));   // Регистрация построителя цепочки обработки
```
...
```csharp
public class MyService(IMediatrPipelineBuilder<Result> pipelineBuilder)
{
    public async Task<string> ProcessAsync()
    {
        ...
        return await pipelineBuilder
            .SetCommand(command)        // Установка основной команды
